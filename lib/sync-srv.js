...
module.exports = function (cfg, apiMetrics) {
  if (!cfg) {
    return;
  }
  config = cfg;
  return sync(apiMetrics);
};

var syncInited = false;
var apiMetrics;
var sync = function (apiMetricsObj) {
  apiMetrics = apiMetricsObj;
  apiMetrics.gauge()(apiMetrics.APP_TITLE + "-control", {method: "syncMain"}, 20);
